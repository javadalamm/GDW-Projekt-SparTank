//Aufgabe 1)
// Ausgabe auf der Konsole
console.log("Aufgabe 1.");
console.log("Ich heiße Tugce Atay");
console.log("Ich heiße Javad Alamdar");
console.log("Ich heiße Hilal Karatay");

console.log("");
//Aufgabe 2)
console.log("Aufgabe 2.");
const max_hoehe = 5;
var aktuelleAnzahlBewertungen = 50; // Teilnahme der Personen
var gespeicherteBewertungen = 250; // alle geben 5 Sterne (50*5)

console.log("Dies ist eine Bewertung");
console.log("Maximale Sterne:" , max_hoehe);
console.log("Anzahl der Bewertung:" , aktuelleAnzahlBewertungen);
console.log("Sterne insgesamt:" , gespeicherteBewertungen);
console.log("");

// Veränderte Werte
aktuelleAnzahlBewertungen = 60
gespeicherteBewertungen = 300

console.log("Dies ist neue Bewertung");
console.log("Maximale Sterne:" , max_hoehe);
console.log("Anzahl der Bewertung:" , aktuelleAnzahlBewertungen);
console.log("Sterne insgesamt:" , gespeicherteBewertungen);
console.log("");

// Zusätzliche Bewertung / Veränderte Werte
// mit lt oder const (-> zuweisen eines anderen Datentyps) 

console.log("Es kommt eine neue Bewertung dazu. Die Werte ändern sich");

// ändern der Werte
aktuelleAnzahlBewertungen = 51 // eine neue Bewertung 
gespeicherteBewertungen = 251 // nur ein Stern Bewertung 

console.log("Anzahl der aktuellen Bewertungen:" , aktuelleAnzahlBewertungen);
console.log("Sterne insgesamt:" , gespeicherteBewertungen);
console.log("");

//Aufgabe 3) Bewertung über die Konsole
console.log("Aufgabe 3.");


let Obst = ["Banana","Apfel","Gurken"];

console.log(Obst.length);



Obst.push("Tomaten");


Obst.forEach(function(item,index, array){
    console.log(item,index);
    });

Obst.pop();

Obst.forEach(function(item,index, array){
    console.log(item,index);
	});
